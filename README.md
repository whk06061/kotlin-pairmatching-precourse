...
- [x] 사용자로부터 기능을 입력받는다.
- [x] 크루의 이름을 String 리스트로 불러온다.
- [x] 페어를 매칭한다.
    - [x] 크루들의 이름 목록을 ListOf<String> 형태로 준비한다. 크루 목록의 순서를 랜덤으로 섞는다.
...
- 해당 코스의 해당 레벨에서 만들어진 모든 페어 정보를 가져와서 겹치지 않게 매칭
...
- 코스와 레벨 마다 객체 생성
- 페어를 매칭해주는 클래스의 메서드로 객체 안에서 해당 레벨 미션의 페어 저장
